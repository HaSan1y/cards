...
async function getsensolData() {
	try {
		const { data: sentencesData, error: sentencesError } = await supabase.from("sentences").select("*").eq("file_id", counter.switchedFiles);

		const { data: solutionsData, error: solutionsError } = await supabase.from("solutions").select("*").eq("file_id", counter.switchedFiles);

		if (sentencesError || solutionsError) {
			throw new Error("Error fetching data from Supabase");
		}

		const sentences = sentencesData.map((row) => row.content);
		const solution = solutionsData.map((row) => row.content);
...
async function postsensolData(event) {
	...
	const { data: sentencesData, error: sentencesError } = await supabase.from("sentences").insert([
		{ content: t1, file_id: counter.switchedFiles },
		{ content: t2, file_id: counter.switchedFiles },
	]);

	const { data: solutionsData, error: solutionsError } = await supabase.from("solutions").insert([{ content: solution, file_id: counter.switchedFiles }]);

	if (sentencesError || solutionsError) {
		console.error("Error writing data to Supabase:", sentencesError || solutionsError);
	} else {
